...
export const adapter = (orm: MikroORM, config: MikroOrmAdapterConfig = {}) => {
  ...
  const createAdapter =
    (em: MikroORM["em"]): AdapterFactoryCustomizeAdapterCreator =>
    () => {
      return {
        ...
        updateMany: async ({ model, where, update }) => {
          const entity = getEntityByModel(orm, model);
          const transformedWhereQuery = transformWhere(where);
          const instances = await em.nativeUpdate(
            entity,
            transformedWhereQuery,
            update as EntityDTO<typeof entity>,
          );
          return instances;
        },
        ...
        findOne: async <T>({
          ...
          where,
          select,
        }: Parameters<DBAdapter["findOne"]>[0]) => {
          const entity = getEntityByModel(orm, model);
          const transformedWhereQuery = transformWhere(where);

          const result = await em.findOne(entity, transformedWhereQuery, {
            fields: select,
          });
        ...
        deleteMany: async ({ model, where }) => {
          const entity = getEntityByModel(orm, model);
          const transformedWhereQuery = where ? transformWhere(where) : {};
          const instances = await em.nativeDelete(
            entity,
            transformedWhereQuery,
          );
          return instances;
